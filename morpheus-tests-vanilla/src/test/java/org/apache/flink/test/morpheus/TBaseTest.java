...
public class TBaseTest {
	...
	protected static final int numTaskManagers = 6;
	protected static final int slotsPerTaskManager = 5;
	protected static final int numSlots = numTaskManagers * slotsPerTaskManager;

	protected static TestingCluster cluster;
	protected static StandaloneClusterClient client;
...
//		config.setInteger("state.replication.placement.bin-packing-cache", 300);
...
//		config.setString(JobManagerOptions.EXECUTION_FAILOVER_STRATEGY, FailoverStrategyLoader.PIPELINED_REGION_RESTART_STRATEGY_NAME);
		...
		client = new StandaloneClusterClient(config, cluster.highAvailabilityServices(), false);
